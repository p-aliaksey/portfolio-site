...
echo "----------------------------------------"
ERROR_COUNT=$(docker logs promtail --since 1m 2>&1 | grep -c "status=400" || echo "0")
echo "Ошибки 400 за последнюю минуту: $ERROR_COUNT"
...
if [ "$ERROR_COUNT" -eq 0 ]; then
    ...
    echo "⚠️  Проблема все еще существует"
fi

echo "✅ Финальный тест завершен - $(date)"
